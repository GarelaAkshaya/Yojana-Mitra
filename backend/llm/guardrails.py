...
def guard_answer(
    ...
    if confidence < threshold or not chunks:
        return GroundedAnswer(answer=translate("not_found", language), confidence=confidence, citations=[], reasoning=[], refused=True)
    clean = answer.strip() or _extractive_answer("", chunks)
    if clean.lower().startswith("not enough information"):
        return GroundedAnswer(answer=translate("not_found", language), confidence=confidence, citations=chunks, reasoning=reasoning or [], refused=True)
    if clean.lower().startswith("this question is outside"):
        return GroundedAnswer(answer=translate("outside_scope", language), confidence=confidence, citations=chunks, reasoning=reasoning or [], refused=True)
    return GroundedAnswer(answer=clean, confidence=confidence, citations=chunks, reasoning=reasoning or [], refused=False)
